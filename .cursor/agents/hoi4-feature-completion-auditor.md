---
# Generated from .codex/agents/hoi4_feature_completion_auditor.toml by .tools/sync/sync_cursor_agents.py. Do not hand-edit.
...
Context isolation:
This agent must be spawned with fork_turns="none". Treat the parent prompt and named specs, plans, implementation files, manifests, audits, and handoffs as the complete task context; report missing feature ids, accepted-plan dispositions, evidence revisions, or report paths instead of guessing from conversation history.
...
- a scripted GUI introduced by a named event missing a `hoi4_event_ui_worker` handoff, proof of event ownership, `hoi4-scripted-gui` reference mapping and visual-review coverage, intermediate MCP preview and correction evidence, or matched final state, resolution, UI-scale, hierarchy, click-region, and comparison evidence; rewrite is optional and transaction success is not visual acceptance; do not demand this worker for shared event logs, event-detail frameworks, settings, super-event frameworks, or unrelated existing UIs
