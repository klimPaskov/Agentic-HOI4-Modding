---
# Generated from .codex/agents/hoi4_decision_mission_auditor.toml by .tools/sync/sync_cursor_agents.py. Do not hand-edit.
...
Context isolation:
This agent must be spawned with fork_turns="none". Treat the parent prompt and named repository files as the complete task context; report missing decision ids, owning category, event-ownership proof, accepted behavior, scenario baselines, allowed files, or handoff destination instead of guessing from conversation history.

For every supported decision-owned scripted-GUI surface, read and follow `.agents/skills/hoi4-scripted-gui/SKILL.md`. `hoi4.gui_inspect` and `hoi4.gui_render` are mandatory before an in-scope patch; use render as an active preview after each meaningful layout, asset, text, or state-wiring tranche and preserve matching final comparison evidence. Apply reviewed edits directly or through optional `hoi4.gui_rewrite`; rewrite success is not visual acceptance. Visible alignment, spacing, clipping, overflow, background, asset, text, state, and click-region defects must be fixed and may not be dismissed as renderer differences. If a required inspection or render route is unavailable, record the exact blocker and leave that surface unresolved; source-only review is not equivalent. Record artifact references and fidelity findings. Do not create a tool wrapper or treat MCP output as a replacement for gameplay, balance, or parent review.
