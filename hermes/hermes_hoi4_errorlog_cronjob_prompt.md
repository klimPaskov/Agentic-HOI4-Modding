...
Profile scripts directory: <Hermes profile scripts path>
...
Coding-agent handoff target: <Codex, another Hermes profile, local file, webhook, chat, etc.>
...
Use system inspection where possible: current repo, Hermes profile/config, HOI4/Steam install locations, Paradox user-data locations, and available coding-agent tools. Ask the user for any value that cannot be discovered confidently.
...
Create the Hermes cronjob after scripts are in place:
...
Verify with real commands appropriate for the detected OS and Hermes install:

```text
1. Syntax-check/compile the worker.
2. Run the worker with --dry-run.
3. Confirm Hermes cron status.
4. Confirm the new cronjob appears in the job list.
5. Trigger the cronjob manually.
6. Inspect <temp log dir>/watchdog.log.
```
...
- detach the worker from the cron launcher so the game run is not limited by Hermes cron runtime
