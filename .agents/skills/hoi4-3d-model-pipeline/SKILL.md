...
- required action roles, runtime consumer, baseline planned paid operations, extra-recovery credit limit, and extra paid-attempt limit; failure-driven recovery stops only for provider refusal, unavailable capability, or insufficient credits
...
The ordinary planned paid path and bounded failure-driven provider recovery are pre-authorized. Do not ask the user for confirmation before the initial model generation or before the remesh, retexture, rigging, conversion, required animation, and task-bounded recovery calls needed to complete an accepted model package. Check the live balance before every paid tranche and record estimates, operation names, attempt numbers, and consumed credits, but balance checks and routine credit use are not confirmation gates.
