...
For licensed or explicitly user-authorized source artwork, use ImageGen to create one substantially original, model-ready refinement informed by the selected concept traits. The refinement may clarify or complete model readability and adapt pose, composition, silhouette, background, or other production-facing presentation while preserving the selected traits and intended unit identity; it need not be pixel-faithful. Reject a source that cannot support a defensible refinement without losing the selected identity.

Keep the untouched source, source-informed refined input, refinement prompt, processing record, source-to-refinement checksums, and comparison as separate evidence surfaces. Obtain explicit parent or user approval of the comparison before the Meshy call. Each Meshy geometry task receives exactly one approved clean input image: a weapon-free body input and a separate firearm-only input for each required gun. Source pages, untouched sources, sheets, collages, turnarounds, contact sheets, comparisons, and diagnostic views remain evidence only. If alpha is required, request native transparent output; a checkerboard or painted transparency is not an alpha channel.

Creating a source-free reference is allowed only after a reasonable documented search across the eligible artwork families finds no suitable source and the parent or user explicitly requests that specific fallback. Ordinary approval to create the 3D model authorizes the source-informed refinement route, not a source-free replacement design. Record the fallback approval, prompt, model or route, output checksum, and restrictions.
