...
description: Use when designing, researching, wiring, or documenting major HOI4 mod super-events.
---

# HOI4 Super Events

Use this skill when a HOI4 modding task creates, updates, researches, or wires a super-event.
...
A the mod super-event is a major presentation moment.

It should mark a major event, major escalation, defeat aftermath, or campaign-ending branch that deserves stronger treatment than a normal popup.
...
- settings-aware or mod-specific playback
- event docs
- planning workbook entry if relevant

Do not wire only one part of a super-event. The image, text, quote, audio, event effect, docs, and planning workbook must agree.
...
- escalated minor events that become major enough for super-event treatment
- campaign-ending branch super-events
- defeat aftermath super-events
- major faction or international-order announcements
...
Use `hoi4-events` for the ordinary event-side implementation contract:

- event script
- event firing path
- on_action, decision, focus, or scripted-effect hooks
- optional mod-specific registration hooks when the repository has them
- docs and manifests
...
Super-event images may be sourced or generated. Prefer generated images for fictional, alternate-history, symbolic, supernatural, extreme-route, or emotionally specific moments. use sourced images when the visual must depict real historical material.
...
| Need | Spawn |
| --- | --- |
| Main quote candidates, wording verification, attribution, source confidence, and quote recommendation | `hoi4_super_event_text_researcher` |
...
The quote, remark, audio, and image subagents produce research notes, final files where applicable, and handoff notes. They do not edit event files, localisation files, `.gfx` files, GUI files, sound definition files, or planning workbook rows unless the parent prompt explicitly expands their scope.
...
Do not reveal a hidden campaign-endinging branch too early.
...
Use the research workflow defined by `AGENTS.md` to verify the exact wording and source of any referenced line.
...
Use the research workflow defined by `AGENTS.md` to find and verify real quotes.
...
Use the research workflow defined by `AGENTS.md` for quote research.
...
Use the research workflow defined by `AGENTS.md` to find and verify cultural references.
...
- existing `music/*.ogg`
- existing `sound/*.wav`
- `music/<mod_namespace>_super_event_music.asset`
- `music/<mod_namespace>_super_event_music.txt`
- `sound/<mod_namespace>_sound.asset`
- `music/<mod_namespace>_music_track_list.html`
...
Use the existing repository settings-aware or mod-specific playback helper. Do not bypass it.
...
When implementing audio, follow the existing repository audio pattern.

The implementation should keep these aligned:

- audio file
- audio id
- `<current_super_event_audio_variable>`
- settings-aware or mod-specific playback helper
- super-event slot
- localisation
- documentation
...
Use the settings-aware or mod-specific playback helper rather than bypassing it.
...
- `music/<mod_namespace>_music_track_list.html` row with the final track and super-event ID or IDs
...
- `music/<mod_namespace>_super_event_music.txt` includes the final super-event track or a representative helper song id for that track
- the sound definitions point to the correct file or intended sound wrapper
- the super-event points to the correct audio id
- `music/<mod_namespace>_music_track_list.html` documents every super-event track and shows the super-event ID or IDs using it
...
2. sets `<current_super_event_audio_variable>`
3. uses the settings-aware or mod-specific playback helper
...
8. updates docs
9. updates docs or optional planning workbook records if relevant
...
## 21. Campaign-ending super-events

Campaign-ending super-events need stricter alignment.

When a super-event represents a campaign-ending branch:

- guard the branch with the campaign-ending rules from `hoi4-events`
...
- make the docs and planning workbook agree
- choose quote and audio with finality in mind

A campaign-ending super-event should feel like an end-state, not a normal escalation.
...
- when it fires
- what event state, branch, or escalation variant triggers it
- what slot it uses
- what image it uses
...
21. `music/<mod_namespace>_super_event_music.txt` includes the final super-event track or a representative helper song id for that track.
...
24. `<current_super_event_audio_variable>` is set correctly.
...
33. Docs or optional planning workbook records are updated if relevant.
