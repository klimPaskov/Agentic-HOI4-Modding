...
Reward-only changes that deliberately preserve layout use inspect, render, and comparison evidence; they do not need an automatic layout rewrite. An invalid or empty identity-preserving rewrite request is a precise schema failure, not authority to invoke compact or automatic layout outside the requested scope. Inspect the actual schema and correct the request only within accepted scope. If the required identity-preserving operation is unavailable, report that blocker and stop writes. Capture an immutable before snapshot, preserve accepted non-layout edits, and roll back only proven tool-generated drift through normal `apply_patch`, recording rejected and restored evidence. Compare focus IDs, count, coordinates, and prerequisites, and distinguish `HEAD` from the dirty index before accepting the result.
...
For every in-scope focus-weight audit, route baseline and post-patch comparison evidence through `hoi4_ai_probability_auditor`. Establish named baseline campaign scenarios before any patch with `hoi4.probability_inspect`, `hoi4.probability_evaluate`, and `hoi4.probability_sweep` as appropriate. The focus owner chooses the intended balance and applies the source patch; the auditor remains read-only. After the patch, require `hoi4.probability_compare` against the same named scenarios and use `hoi4.probability_simulate` only for explicitly declared uncertain inputs. Use `hoi4.probability_render` when a ranking, matrix, sensitivity, comparison, or unresolved view is clearer than a compact trace. The focus adapter models the verified independent score race, not weight divided by the sum of weights. Supply the complete available-focus pool and declare prerequisite, bypass, strategy, and external factors; keep anything the analyzer cannot resolve visible as uncertainty. Use the result as balance evidence while this skill retains ownership of route design and tuning decisions.
