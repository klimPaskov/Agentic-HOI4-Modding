...
When the weakness is weighted AI, timing, or random selection, turn the intended improvement into named scenarios and measurable ordering, timing, dominance, or starvation expectations. Route the mandatory read-only pass through `hoi4_ai_probability_auditor`: establish the baseline with `hoi4.probability_inspect`, select evaluation, sweep, simulation, sequence, comparison, or render evidence according to the declared scenario, let the owner apply any bounded patch, and require `hoi4.probability_compare` against the same scenarios afterward. The auditor does not choose balance targets or patch source. If the required MCP route is unavailable, record the exact blocker and leave the improvement unvalidated; source-only review is not equivalent. Do not ask the implementation agent to prove an exact probability from an incomplete candidate pool or undeclared external state.
