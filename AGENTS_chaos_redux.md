...
### Dual-Runtime Workflow (Codex + Cursor)

This repository runs parallel Codex and Cursor runtimes that never author each other's configuration. Codex is the primary workflow and loads `.codex/config.toml` plus `.codex/agents/*.toml`. Cursor loads `.cursor/mcp.json`, `.cursor/rules/`, and generated `.cursor/agents/*.md` files as native Cursor project subagents. Everything else (`.agents/skills/`, AGENTS.md, docs, specs, plans, handoffs, and the `.tools/3d_pipeline/wrappers/` MCP wrappers) is shared and runtime-agnostic.

- Canonical source: the Codex agent TOMLs in `.codex/agents/` are the single authoring source for every custom subagent. Cursor definitions are generated files. Never hand-edit a generated `.cursor/agents/*.md` file; edit the TOML source and rerun `python .tools/sync_cursor_agents.py`.
- Name mapping: Codex identifiers use snake_case (`chaosx_repo_explorer`). Cursor requires lowercase letters and hyphens, so the generated name is the hyphen-case equivalent (`chaosx-repo-explorer`). Cursor maps live in `.cursor/agent-map.md`. Routing rules in this file and in skills always use the canonical snake_case identifier.
...
- MCP registration: the Codex side registers servers in `.codex/config.toml`. Cursor registers the same production servers (`hoi4_agent_tools`, `meshy`, `blender_hoi4`) in `.cursor/mcp.json`. Codex-only keys such as approval modes and tool allowlists have no Cursor equivalent; the matching discipline is carried by the subagent prompts instead. `blender_lab_dev` remains a Codex-only diagnostic server.
- Anti-interference rules: during a Cursor session treat `.codex/**` as read-only reference. During a Codex session leave generated `.cursor/agents/**` untouched. Any subagent instruction change lands in the TOML first, then propagates through the Cursor sync script.
