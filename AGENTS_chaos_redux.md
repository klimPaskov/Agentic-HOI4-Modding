...
- Use `chaos-redux-comfyui` for sourced or grounded portraits: archive and wire the source portrait, then install the HOI4-style final when the user supplies it. RunPod is user-only and must never be operated by an agent.
...
- Use `chaos-redux-3d-model-pipeline` for Chaos Redux HOI4 3D models and required sound/counter handoffs: Meshy 6 by default, sourced unit audio, and vanilla-green counters. Planned model/animation spend needs no confirmation; ask only for extra paid recovery after failure or rejection. Route to `chaosx_3d_model_pipeline` with `fork_context=false`.
...
- Use `chaosx_portrait_creator` to validate and install a user-supplied HOI4-style replacement for an archived sourced portrait. It never operates RunPod or generates the portrait.
- For asset animation, route 2D frame sheets to `chaos-redux-frame-animation` and skeletal `.anim` actions to `chaos-redux-3d-model-pipeline`. 3D prompts require Meshy 6, one approved image, vanilla scale, packed materials, reimport proof, parent-owned wiring, sourced-audio provenance/checksums/sync, and bespoke green counters. Normal spend needs no confirmation; only failure-driven extra recovery does.
...
16. Unapproved fallbacks are forbidden and MUST ALWAYS be discussed with the user. A wired sourced portrait remains an explicitly pending source placeholder until the user supplies its HOI4-style replacement.
...
For sourced or grounded portraits, save the attributed source under `docs/assets/portraits/<event_id>_<event_slug>/`, create and wire its source placeholder, and keep the runtime basename stable. The user alone creates the HOI4-style portrait externally. When the user supplies it, the agent validates it, converts it to DDS, and replaces the placeholder. Agents never operate RunPod. Non-sourced fictional or impossible portraits use native ImageGen.
