...
- Use `chaos-redux-3d-model-pipeline` for Chaos Redux HOI4 3D models and required sound/counter handoffs: Meshy 7 exclusively, sourced unit audio, and vanilla-green counters. Planned model/animation spend needs no confirmation; ask only for extra paid recovery after failure or rejection. Route to `chaosx_3d_model_pipeline`.
...
### Dual-Runtime Workflow (Codex + Qoder + Cursor)

This repository runs parallel agent runtimes that never author each other's configuration. Codex is the primary workflow. The Codex runtime loads `.codex/config.toml` and `.codex/agents/*.toml`. The Qoder runtime loads `.qoder/mcp.json` and `.qoder/agents/*.md`. The Cursor runtime loads `.cursor/mcp.json`, `.cursor/rules/`, and generated `.cursor/agents/*.md` files as native Cursor project subagents. Everything else (`.agents/skills/`, AGENTS.md, docs, specs, plans, handoffs, and the `.tools/3d_pipeline/wrappers/` MCP wrappers) is shared and runtime-agnostic.

- Canonical source: the Codex agent TOMLs in `.codex/agents/` are the single authoring source for every custom subagent. Qoder and Cursor definitions are generated files. Never hand-edit a generated `.qoder/agents/*.md` or `.cursor/agents/*.md` file; edit the TOML source and rerun `python .tools/sync_qoder_agents.py` and `python .tools/sync_cursor_agents.py`.
- Name mapping: Codex identifiers use snake_case (`chaosx_repo_explorer`). Qoder and Cursor both require lowercase letters and hyphens, so the generated name is the hyphen-case equivalent (`chaosx-repo-explorer`). Cursor maps live in `.cursor/agent-map.md`. Routing rules in this file and in skills always use the canonical snake_case identifier.
...
- MCP registration: the Codex side registers servers in `.codex/config.toml`. Qoder and Cursor register the same production servers (`hoi4_agent_tools`, `meshy`, `blender_hoi4`) in `.qoder/mcp.json` and `.cursor/mcp.json`. Codex-only keys such as approval modes and tool allowlists have no Qoder or Cursor equivalent; the matching discipline is carried by the subagent prompts instead. `blender_lab_dev` remains a Codex-only diagnostic server.
- Anti-interference rules: during a Qoder or Cursor session treat `.codex/**` as read-only reference. During a Codex session leave `.qoder/**` and generated `.cursor/agents/**` untouched. Any subagent instruction change lands in the TOML first, then propagates through the sync scripts.
