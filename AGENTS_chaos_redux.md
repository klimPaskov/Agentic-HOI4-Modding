...
This repository runs parallel agent runtimes that never author each other's configuration. Codex is the primary workflow and loads `.codex/config.toml` plus `.codex/agents/*.toml`. Qoder loads `.qoder/mcp.json` and generated `.qoder/agents/*.md`, Cursor loads `.cursor/mcp.json`, `.cursor/rules/`, and generated `.cursor/agents/*.md`, OpenCode loads its project-local `.opencode/` configuration and generated `.opencode/agent/*.md`, and Claude Code loads `CLAUDE.md`, `.claude/settings.json`, root `.mcp.json`, and generated `.claude/agents/*.md`. Everything else (`.agents/skills/`, AGENTS.md, docs, specs, plans, handoffs, and the `.tools/3d_pipeline/wrappers/` MCP wrappers) is shared and runtime-agnostic.

- Canonical source: the Codex agent TOMLs in `.codex/agents/` are the single authoring source for every custom subagent. Qoder, Cursor, OpenCode, and Claude Code definitions are generated files. Never hand-edit them; edit the TOML source and rerun `python .tools/sync/sync_qoder_agents.py`, `python .tools/sync/sync_cursor_agents.py`, `python .tools/sync/sync_opencode_agents.py`, and `python .tools/sync/sync_claude_agents.py`.
- Name mapping: Codex identifiers use snake_case (`chaosx_repo_explorer`). Alternate runtimes use the hyphen-case equivalent (`chaosx-repo-explorer`). Runtime mapping files are generated with the agent definitions. Routing rules in this file and in skills always use the canonical snake_case identifier.
- Cursor subagent format: generated files must remain Cursor-native Markdown in `.cursor/agents/<kebab-name>.md` with YAML frontmatter `name`, `description`, and `model: inherit`, followed by the prompt body. Cursor loads every `*.md` in that folder into the Task tool. Do not place README or other non-agent Markdown there. Spawn Cursor subagents by hyphen-case name (`/chaosx-repo-explorer` or the matching Task `subagent_type`). Cursor isolates subagent context; the parent prompt must still be fully explicit. Do not treat `.codex/agents/*.toml` as Cursor subagent prompts.
- Claude Code subagent format: generated files remain Claude-native Markdown in `.claude/agents/<kebab-name>.md` with required `name` and `description` frontmatter plus `model: inherit`. Read-only agents deny `Write`, `Edit`, and `NotebookEdit` while retaining inherited MCP tools. Claude Code uses `CLAUDE.md`, `.claude/settings.json`, and root `.mcp.json`; project MCP servers still require the runtime's one-time trust decision.
- MCP registration: the Codex side registers servers in `.codex/config.toml`. Each alternate runtime registers the same production routes in its own verified project configuration. Codex-only keys such as approval modes and tool allowlists do not transfer automatically; the matching discipline is carried by the subagent prompts instead. `blender_lab_dev` remains a Codex-only diagnostic server.
- Anti-interference rules: during a Qoder, Cursor, OpenCode, or Claude Code session treat `.codex/**` as read-only reference. During a Codex session leave generated alternate-runtime agents untouched. Any subagent instruction change lands in the TOML first, then propagates through all four sync scripts.
...
11. When adding a Chaos Redux land sub-unit, concrete equipment type, special project, special facility, doctrine, or general system, register one package-owned idempotent CXT setup effect through the dynamic extension contract in `docs/testing/chaosx_test_country.md` in the same change. Give the package a modifier-free hidden-idea carrier whose id matches the setup effect before its `_apply` suffix. An additive `on_startup` block must register that carrier token through one bounded existing-country scope so the first console setup consumes it, and an additive `on_daily_CXT` fallback must register and synchronize it for existing saves. Use an additive `on_weekly_CXT` block only for recurring maintenance not already owned by the shared CXT hooks.
12. Document each new script file with an overview at the top. Do it similarly to `chaosx_logic_effects.txt`.
13. Confirm that all decisions and event options or other effects have proper trigger tooltips and effect descriptions.
14. Respect the repository style and naming rules so new content blends with existing Chaos Redux code.
15. For systems that touch or are related to bio/chemical warfare, review any related docs, and verify integration across events, on_actions, decisions, contamination effects, scripted logic, etc.
...
21. Respect the player-facing writing style found in `chaos-redux-events`.
22. Markdown prose must not be hard-wrapped in the middle of a sentence. Keep each sentence on one physical line and place line breaks only between sentences or paragraphs.
...
- a named event-owned scripted GUI is missing its `chaosx_event_ui_worker` handoff or mandatory MCP before-and-after layout evidence
...
Native ImageGen transparency is the default for every asset family whose unused canvas is transparent in the inspected HOI4 consumer, including icons, counters, emblems, overlays, decorative UI pieces, and transparent animation frames. Request a real transparent background in the initial ImageGen call and preserve the alpha channel through processing, frame assembly, resizing, and DDS conversion. Background removal is a fallback only for failed native-alpha output or an inherited, sourced, or user-provided image with an unwanted opaque backdrop; record the fallback and validate the repaired edges. Full-canvas scene art, flags, portraits, and painted panels keep the background treatment required by their actual consumer and reference family.

Route every character portrait to `chaosx_portrait_creator`. It finds and archives attributed sources for grounded subjects, creates and wires source placeholders, and validates and installs the HOI4-style final supplied by the user; agents never operate RunPod. For fictional or impossible subjects, it invokes native ImageGen and completes processing, DDS conversion, portrait-specific wiring, manifest evidence, and handoff itself.
...
Use `chaos-redux-subagents` for asset routing. The 3D worker owns model production, sourced-audio synchronization, and the counter handoff. `chaosx_icon_artist` creates original green counters after exact vanilla definition/DDS and reference inspection. The parent owns runtime wiring, documentation, and final validation.
