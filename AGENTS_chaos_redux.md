...
- For asset animation, route 2D frame sheets to `chaos-redux-frame-animation` and skeletal `.anim` actions to `chaos-redux-3d-model-pipeline`. 3D prompts require Meshy 6, one approved image, vanilla scale, packed materials, reimport proof, parent-owned wiring, sourced-audio provenance/checksums/sync, and bespoke green counters from exact vanilla and skill references. Normal spend needs no confirmation; only failure-driven extra recovery does.
...
Use `chaos-redux-subagents` for asset routing. The 3D worker owns model production, sourced-audio synchronization, and the counter handoff; it never creates audio. `chaosx_icon_artist` creates original green counters after exact vanilla definition/DDS and reference inspection. The parent owns runtime wiring, documentation, and final validation.
