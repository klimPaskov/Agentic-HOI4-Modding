...
- Anti-interference rules: during a Qoder, Cursor, or OpenCode session treat `.codex/**` as read-only reference. During a Codex session leave generated alternate-runtime agents untouched. Any subagent instruction change lands in the TOML first, then propagates through all three sync scripts.
