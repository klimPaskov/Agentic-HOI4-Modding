...
- Use `chaos-redux-3d-model-pipeline` for Chaos Redux HOI4 3D models and their mandatory handoffs: Meshy 6 by default, Internet-sourced unit audio, and bespoke vanilla-green counters. Planned model and animation credit use is pre-authorized; ask only before extra paid recovery after failure or rejection. Route bounded production to `chaosx_3d_model_pipeline` with `fork_context=false`.
...
- For asset animation, route 2D frame sheets to `chaos-redux-frame-animation` and skeletal `.anim` actions to `chaos-redux-3d-model-pipeline`. 3D prompts must require Meshy 6, one approved image, vanilla scale, material packing, reimport proof, parent-owned wiring, Internet-sourced audio with provenance/checksums/sync points, and bespoke green counters based on exact installed-vanilla and skill-local references. Planned operations need no credit confirmation; only failure-driven extra paid recovery does.
...
Use `chaos-redux-subagents` for asset routing. The 3D worker owns bounded model production, sourced-audio research/synchronization, and the counter handoff; it never creates audio. `chaosx_icon_artist` creates original green counters only after exact vanilla definition/DDS and matching references are inspected. The parent owns `.gfx`, `.asset`, entity, sound, gameplay, localisation, runtime wiring, documentation, and final validation.
