...
- Use `hoi4-3d-model-pipeline` for bounded custom HOI4 model production with Meshy 6 as the default generation model, provider lineage, Blender processing, PDX textures, rigs, skeletal actions, sourced custom-unit sound design, bespoke vanilla-green custom-unit counters, `.mesh`/`.anim` export, reimport proof, and runtime handoffs. Normal model and required animation credit use is pre-authorized; ask only before extra paid recovery caused by a failed or rejected attempt, and otherwise do not ask for credit confirmation.
...
- Technology work: use the technology and doctrine inspect, render, and compare routes only when the installed server actually exposes them. The package revision documented by this starter kit has no Technology Tree Viewer; do not invent a route or claim viewer evidence, and treat work that requires that unavailable route as blocked or unresolved under the MCP rule below.
